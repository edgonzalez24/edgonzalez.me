  ...
  <u-page :ui="{ center: 'lg:col-span-7!' }" class="relative mx-auto max-w-screen-2xl md:px-8 pb-10 pt-24">
    <template #right>
      <u-page-aside :ui="{ root: 'lg:col-span-3!' }">
        <u-page-anchors 
          :links="[{ label: 'All articles', icon: 'material-symbols:article-rounded', to: '/articles/' }]"
        />
    ...
    <u-page-header :title="data?.title" :description="data?.description" headline="Blog">
      <div class="flex items-end flex-wrap gap-4 justify-between mt-4">
        ...
        <div class="flex flex-row items-center gap-4">
          ...
          <p class="flex flex-row items-center gap-1 typ-sublabel">
            <icon name="material-symbols:alarm-rounded" class="text-primary"></icon> {{ readingTimeText }}
          </p>
        </div>
      </div>
    </u-page-header>
    ...
    <u-page-body>
      ...
      <u-blog-posts id="related-articles">
        <u-blog-post
          ...
          :date="article.date" :to="article.path" variant="subtle"
        />
      </u-blog-posts>
...
const updateMeta = () => {
  useSchemaOrg([
    defineArticle({
      ...
      image: getTumbnail(data.value?.thumbnail ?? ''),
